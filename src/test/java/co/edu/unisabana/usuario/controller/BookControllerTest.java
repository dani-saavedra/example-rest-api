...
public class BookControllerTest extends AbstractTest {
    ...
    @Test
    public void Given_BasicTest_When_callData_Then_register() {
        BookDto dto = new BookDto("Book 1", 2003, "Pedro", "Commercial", "suave");

        ResponseEntity<BookReponse> result = restTemplate.postForEntity(PATH_REGISTER_BOOK,
                dto, BookReponse.class);
        assertEquals("Nuevo libro registrado", result.getBody().getData());
    }

    @Test
    public void Given_BasicTest_When_callData_Then_update() {
        BookDto dto = new BookDto("Book 1", 2003, "Pedro", "Commercial", "suave");
        restTemplate.postForEntity(PATH_REGISTER_BOOK, dto, BookReponse.class);


        ResponseEntity<BookReponse> result2 =
                restTemplate.postForEntity(PATH_REGISTER_BOOK, dto, BookReponse.class);
